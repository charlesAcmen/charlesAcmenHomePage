...
- 正对海岸、只向前推进的滚轮镜头；底部滑杆和键盘也可控制进度。手机/平板在进场途中单指上滑推进，进入工作室后单指拖动进行水平 360° 环视，双指张合可随时推进或后退。
- 白底、黑色双层线条和少量手绘扰动组成的海岸、道路、棕榈树、车辆与 Art Deco 建筑。
- 参考 Miami Beach Breakwater Hotel 轮廓手工搭建的标志性酒店。
- 加深并抬高的近方形彩色工作室：镜头最终进入室内；点击空白处锁定鼠标以无限环视，中心准星决定卡片与显示器的点击目标。锁定时点击显示器会进入独立播放器模式并释放鼠标，该模式不会自动重新请求 Pointer Lock；点击显示器外的空白画布才返回环视。
...
- 默认开启的声音场景：平静水浪作为全局底声，六辆车混合三档引擎循环；车辆声音带左右声像和距离衰减，并按照滚动产生的实际移动速度平滑改变响度与转速，停止时不再持续堆叠怠速低频。
...
- 播放器右上角保留 `OPEN ON YOUTUBE` 入口；当嵌入播放器遇到登录、机器人验证或第三方 Cookie 限制时，可在正常浏览器标签中继续。
...
桌面开发者工具适合检查断点、DPR、横竖屏和 CPU/网络降速，但触摸手感、移动 GPU、浏览器地址栏、自动播放和多指操作最终必须在真机确认。让同一局域网或 Tailscale 内的手机直接访问开发机：

```powershell
npm run dev:mobile
```

终端会输出可从其他设备访问的 Network 地址。在手机浏览器中打开该地址并进入 `#coast`；Windows 防火墙首次询问时只允许专用网络。Android 还可以开启 USB 调试，在桌面 Chrome 的 `chrome://inspect/#devices` 中检查真机页面、Console、Network 和 Performance。若不希望开放局域网端口，可在该页面把手机 `3000` 端口转发至开发机 `localhost:3000`。
...
app/
  ...
  coast/
    ...
    pointer-lock-controller.ts   环视、播放器模式与浏览器重锁冷却状态机
    touch-controls.ts            触屏手势状态机、轻点判定与误触抑制
    viewport-profile.ts          手机/平板取景角度和像素比预算
...
- 触屏设备根据屏幕尺寸把像素比进一步限制到 `1.35–1.5`；竖屏程度越高，垂直 FOV 会平滑扩大到最多 `72°`，室内终点也会沿原中轴后退最多 `3.2` 个世界单位，在不改变直进镜头叙事的前提下多露出两侧墙面。
- 只有鼠标或摄像机变化时才重新做卡片射线检测。
- 触摸轻点只在抬手时进行九点小范围拾取；连续拖动不执行额外的多点 raycast，也不会在逐帧循环分配手势对象。
...
工作室视频通过 YouTube 标准 iframe 从官方平台播放，以便浏览器在允许第三方 Cookie 时复用用户登录状态；不下载、不转存，也不把远程视频制作为 Three.js 纹理。视频能否播放仍取决于访客网络、Cookie 策略、YouTube 可用性以及视频所有者的嵌入设置。
